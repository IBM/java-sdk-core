...
public class RetryTest extends BaseServiceUnitTest {
  ...
  public void testDoNotRetry501() {
    ...
    server.enqueue(new MockResponse().setResponseCode(501).addHeader(CONTENT_TYPE, HttpMediaType.APPLICATION_JSON)
        .setBody("{\"error\": \"" + message + "\"}"));
  ...
  public void testValidRetryAfter() {
    ...
    String message = "The request failed because the moon is full.";
    server.enqueue(new MockResponse().setResponseCode(504).addHeader(CONTENT_TYPE, HttpMediaType.APPLICATION_JSON)
        .addHeader("Retry-After", "5").setBody("{\"error\": \"" + message + "\"}"));
    server.enqueue(new MockResponse().setResponseCode(200).addHeader(CONTENT_TYPE, HttpMediaType.APPLICATION_JSON)
        .setBody("{\"success\": \"awesome\"}"));
  ...
  public void testInvalidRetryAfter() {
    ...
    server.enqueue(new MockResponse().setResponseCode(504).addHeader(CONTENT_TYPE, HttpMediaType.APPLICATION_JSON)
        .addHeader("Retry-After", "-1").setBody("{\"error\": \"this is not valid\"}"));
    server.enqueue(new MockResponse().setResponseCode(200).addHeader(CONTENT_TYPE, HttpMediaType.APPLICATION_JSON)
        .setBody("{\"success\": \"awesome\"}"));
  ...
  public void testHttpTimeRetry() {
    DateTimeFormatter f = DateTimeFormatter.ofPattern("EEE, dd MMM yyyy HH:mm:ss zzz", Locale.ENGLISH)
        .withZone(ZoneOffset.UTC);
    Instant now = Instant.now().plusSeconds(3);
    String after = f.format(now);

    server.enqueue(new MockResponse().setResponseCode(504).addHeader(CONTENT_TYPE, HttpMediaType.APPLICATION_JSON)
        .addHeader("Retry-After", after).setBody("{\"error\": \"Be patient!\"}"));
    server.enqueue(new MockResponse().setResponseCode(200).addHeader(CONTENT_TYPE, HttpMediaType.APPLICATION_JSON)
        .setBody("{\"success\": \"awesome\"}"));
